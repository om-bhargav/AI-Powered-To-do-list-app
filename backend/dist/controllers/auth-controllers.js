...
import { hashPassword, comparePassword, } from "../handlers/password-handlers.js";
...
export async function login(req, res) {
    ...
    if (data && (await comparePassword(password, data.password))) {
        const token = jwt.sign({ _id: data.id, name: data.name, email: data.email }, process.env.JWT_SECRET, {
            expiresIn: "3h",
        });
